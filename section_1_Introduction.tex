...
More importantly, we design a specially customized algorithm, namely, NOSSGA (Normalized Objectives' Sum Sorting Genetic Algorithm), by modifying a popular EMO algorithm, NSGAII~\cite{deb2002fast}. The final population of NOSSGA contains better species trees than that of NSGAII.  %(Section~\ref{sec:method}).
We consider three optimization criteria: quartet consistency (ASTRAL), maximum pseudo-likelihood (MP-EST), and triplet consistency (STELAR). We assess the validity of our approach on a collection of challenging simulated dataset. Our results suggest that NOSSGA can produce a tree-space which contains substantially more accurate trees than the trees estimated by optimizing the individual criterion. 

%Furthermore, we explored the SPR (subtree pruning and regrafting) neighborhood of ASTRAL and MP-EST estimated trees and showed that they do not contain the highly accurate trees that can be found in the population generated by NOSSGA using multi-objective optimization.
