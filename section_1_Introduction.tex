...
More importantly, we designed a specially customized algorithm, namely, NOSSGA (Normalized Objectives' Sum Sorting Genetic Algorithm), by simplifying NSGAII which is a popular EMO algorithm, whose final population contains better candidate species trees than that of NSGAII.  %(Section~\ref{sec:method}).
